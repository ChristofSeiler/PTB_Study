...
install.packages(c("xml2","ggfortify","magrittr","ff","foreach","doMC"))
```
...
```{r}
params$plan
if(params$plan == "plan_1") {
  # plan 1: Differences at baseline (only using the mock samples, case vs. control)
  eset = eset[,eset$Case.Control == "Mock"]
} else if(params$plan == "plan_2") {
  # plan 2: Differences of ratios between + and - (H1N1 and mock), case vs. control
  array_name = as.character(eset[["array_name"]])
  ...
  pairs = data.frame(mock=array_name[mock_ids],
                     h1n1=array_name[h1n1_ids],
                     mock_id=mock_ids,
                     h1n1_id=h1n1_ids,
                     mock_cc=eset[["Infection"]][mock_ids],
                     h1n1_cc=eset[["Infection"]][h1n1_ids],
                     Infection=eset[["Infection"]][mock_ids])
...
```{r}
file_name_results = paste0(params$plan,"_results.csv")
file_name_results
write.csv(gene_table,file = file_name_results)
```
