...
install.packages(c("xml2","ggfortify","magrittr","ff","foreach","doMC","statmod","fdrtool"))
```
...
cm = makeContrasts(H1N1_vs_Mock_for_Preterm-H1N1_vs_Mock_for_Term,levels = design)
fit = contrasts.fit(fit, cm)
fit = eBayes(fit)
gene_table = topTable(fit, 
                      coef="H1N1_vs_Mock_for_Preterm - H1N1_vs_Mock_for_Term", 
                      adjust="BH",
                      number = nrow(fit))
hist(gene_table$P.Value)
# re-estimate the p-value
gene_table_new = fdrtool(gene_table$t, statistic= "normal", plot = T)
hist(gene_table_new$pval)
sum(p.adjust(gene_table_new$pval, method = "BH") < 0.05)
```
