...
if(length(V(module)) > 0) {
  ...
  module_df %<>% left_join(mapped, by = c("label" = "symbol"))
  ggplot(module_df, aes(x = x, y = y, xend = xend, yend = yend)) +
    geom_edges() +
    geom_nodelabel_repel(aes(label = label), 
                         box.padding = unit(1, "lines"),
                         alpha = 0.3) +
    geom_nodes(aes(shape = sign_score, color = logFC), size = 4) +
...
tb_pways = lapply(all_pathways, function(pway) {
  ...
  if(length(gene_desc) > 0) {
    ...
    overlap = mean(gene_symbol %in% unique(module_df$label))
  } else {
    overlap = 0
  }
  tibble(
