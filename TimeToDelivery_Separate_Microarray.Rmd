...
```{r plot_network, fig.width=12, fig.height=12}
if(length(V(module)) > 0) {
  ...
  ggplot(module_df, aes(x = x, y = y, xend = xend, yend = yend)) +
    ...
    geom_nodelabel_repel(aes(label = symbol), 
                         ...
                         alpha = 0.3) +
    geom_nodes(aes(shape = sign_score, color = t_statistic), size = 4) +
...
plot_node = function(node_id) {
  ...
  ggplot(tb_expr, aes(res_time_to_delivery, res_expr)) +
    ...
    ggtitle(paste0(module_df$symbol[node_id],
                  ...
                  ")")) +
    xlab("residual of lm(time_to_delivery ~ gestage_delivery)") +
    ylab("residual of lm(expr ~ gestage_delivery)")
}
