...
plot_node = function(node_id) {
  if(length(node_id) == 0) stop("not a valid node")
  probe_id = e2s[which(e2s$symbol == module_df[node_id,"symbol"]), "probe_id"]
  tb_expr = tibble(
  ...
  ggplot(tb_expr, aes(res_time_to_delivery, res_expr)) +
    ...
    geom_point(size = 2) +
    scale_x_reverse() +
    ggtitle(paste0(module_df$symbol[node_id],
                  " (t-statistic = ",
                  round(module_df$t_statistic[node_id],digits = 2),
                  ")")) +
    xlab("resid(lm(time_to_delivery ~ gestage_delivery)") +
    ylab("resid(lm(expr ~ gestage_delivery))")
}
plot_node(which.min(module_df$t))
plot_node(which.max(module_df$t))
```
