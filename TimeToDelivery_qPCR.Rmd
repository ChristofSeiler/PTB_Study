...
plot_reverse = TRUE
coefs = coef(lm_fit)
gmine = ggplot(qpcr_data_RGS13, aes(x = time_to_delivery, y = expr, color = treatment)) + 
  geom_point(size = 2)
if(plot_reverse) {
...
gmine + 
  geom_abline(intercept = coefs["(Intercept)"] +
                mean(qpcr_data_RGS13$gestage_delivery)*coefs["gestage_delivery"],
              slope = flip_slop*coefs["time_to_delivery"],
              color = used_colors[1], size = 1) +
  ggtitle("RGS13")
ggsave(filename = "time_to_delivery_RGS13.png",width = 4,height = 3,dpi = 600)
```
