---
title: "Time to Delivery on Follow-Up qPCR Data"
...
```{r change_RGS13,fig.width=4,fig.height=3}
lm_fit = lm(expr ~ gestage_delivery, qpcr_data_RGS13)
qpcr_data_RGS13 %<>% mutate(residuals = residuals(lm_fit))
qpcr_data_RGS13 %<>% mutate(time_to_delivery_fac = if_else(time_to_delivery < 10,
                                                           true = "< 10",
                                                           false = "10 to 20"))
qpcr_data_RGS13 %<>% mutate(time_to_delivery_fac = if_else(time_to_delivery > 20,
                                                           true = "> 20",
                                                           false = time_to_delivery_fac))
tb = qpcr_data_RGS13 %>% 
  group_by(treatment,time_to_delivery_fac) %>% 
  summarize(mean = mean(residuals))
tb$time_to_delivery_fac = factor(tb$time_to_delivery_fac, 
                                 levels = c("> 20","10 to 20","< 10"))
ggplot(tb,aes(x = time_to_delivery_fac, 
              y = mean, 
              color = treatment,
              group = treatment)) + 
  geom_point(size = 4) + 
  geom_line(size = 1) + 
  xlab("time to delivery in weeks") +
  ylab("mean residual expr") +
  ggtitle("RGS13")
ggsave(filename = "change_RGS13.png",width = 4,height = 3,dpi = 600)
```
...
lm_fit = lm(expr ~ treatment*time_to_delivery + 
...
gtimeto = interact_plot(lm_fit, pred = "time_to_delivery", modx = "treatment",
                        plot.points = TRUE, jitter = 0) + 
  ggtitle("IFNL (treatment:time_to_delivery)") + scale_x_reverse()
gtimeto
interact_plot(lm_fit, pred = "gestage_delivery", modx = "treatment",
              plot.points = TRUE, jitter = 0) + 
  ggtitle("IFNL (treatment:gestage_delivery)")
```

Plot interaction using using only `ggplot`. The function `interact_plot` plots intercepts and slopes whit fixed `gestage_delivery` at their mean values.

```{r my_interact_plot, fig.width=8,fig.height=4}
plot_reverse = TRUE
coefs = coef(lm_fit)
gmine = ggplot(qpcr_data_IFNL, aes(x = time_to_delivery, y = expr, color = treatment)) + 
  geom_point(size = 2)
if(plot_reverse) {
  gmine = gmine + scale_x_reverse() 
  flip_slop = -1
} else {
  flip_slop = 1
}
...
intercept = coefs["(Intercept)"] + coefs["gestage_delivery"]*mean_gestage_delivery
...
gmine = gmine + 
  geom_abline(intercept = intercept,
              slope = flip_slop*(slope + coefs["time_to_delivery"]), 
              color = used_colors[1], size = 1) +
  geom_abline(intercept = intercept + h1n1 + coefs["treatmentH1N1"], 
              slope = flip_slop*(slope + coefs["time_to_delivery"] + coefs["treatmentH1N1:time_to_delivery"]),
              color = used_colors[2], size = 1) +
  ggtitle("IFNL (treatment:time_to_delivery)")
plot_grid(gtimeto,gmine)
ggsave(gmine, filename = "interaction_time_to_delivery_IFNL.png",width = 4,height = 3,dpi = 600)
```

Interpretation.

```{r change_IFNL,fig.width=4,fig.height=3}
lm_fit = lm(expr ~ gestage_delivery + 
              treatment + 
              time_to_delivery + 
              gestage_delivery:treatment +
              time_to_delivery*gestage_delivery, 
            data = qpcr_data_IFNL)
qpcr_data_IFNL %<>% mutate(residuals = residuals(lm_fit))
qpcr_data_IFNL %<>% mutate(time_to_delivery_fac = if_else(time_to_delivery < 10,
                                                          true = "< 10",
                                                          false = "10 to 20"))
qpcr_data_IFNL %<>% mutate(time_to_delivery_fac = if_else(time_to_delivery > 20,
                                                          true = "> 20",
                                                          false = time_to_delivery_fac))
tb = qpcr_data_IFNL %>% 
  group_by(treatment,time_to_delivery_fac) %>% 
...
ggplot(tb,aes(x = time_to_delivery_fac, 
  ...
  ylab("mean residual expr") +
  ggtitle("IFNL")
ggsave(filename = "change_IFNL.png",width = 4,height = 3,dpi = 600)
```
