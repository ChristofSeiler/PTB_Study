...
new_matched %<>% mutate(time_to_delivery_calc = 
                          gestage_delivery - gestage_enroll)
...
qpcr_data %<>% rename(gestational_age = gestage_delivery)
...
ggplot(qpcr_data_RGS13,aes(x = gestational_age, y = expr, 
                           color = treatment)) +
  geom_smooth(method = lm, se = FALSE) +
  geom_point() +
  geom_text(aes(label = ifelse(expr > quantile(expr, probs = 0.95), 
...
```{r fit_model_RGS13}
lm_fit = lm(expr ~ time_to_delivery + gestational_age, qpcr_data_RGS13)
summary(lm_fit)
```
...
lm_fit = rfit(expr ~ time_to_delivery + gestational_age, qpcr_data_RGS13)
summary(lm_fit)
```
...
Plot of fitted intercept and slope with `gestational_age` fixed at its mean value.
...
mean_gestage = mean(qpcr_data_RGS13$gestational_age)
gmine + geom_abline(intercept = coefs["(Intercept)"] + 
                      mean_gestage*coefs["gestational_age"],
                    slope = flip_slop*coefs["time_to_delivery"],
...
ggplot(qpcr_data_IFNL,aes(x = time_to_delivery, y = expr, 
  ...
  ggtitle("IFNL") + 
  scale_x_reverse()
ggplot(qpcr_data_IFNL,aes(x = gestational_age, y = expr, 
                          color = treatment)) +
  geom_smooth(method = lm, se = FALSE) +
  geom_point() +
  geom_text(aes(label = ifelse(expr > quantile(expr, probs = 0.95), 
...
lm_fit = lm(expr ~ treatment*time_to_delivery + 
              treatment*gestational_age + 
              time_to_delivery*gestational_age, 
            data = qpcr_data_IFNL)
summary(lm_fit)
```
...
```{r fit_model_rank}
lm_fit = rfit(expr ~ treatment*time_to_delivery + 
              treatment*gestational_age + 
              time_to_delivery*gestational_age, 
            data = qpcr_data_IFNL)
summary(lm_fit)
...
gtimeto = interact_plot(lm_fit, pred = "time_to_delivery", 
  ...
  ggtitle("IFNL") + scale_x_reverse()
gtimeto
interact_plot(lm_fit, pred = "gestational_age", modx = "treatment",
              plot.points = TRUE, jitter = 0) + 
  ggtitle("IFNL")
```

Plot interaction using using only `ggplot`. The function `interact_plot` plots intercepts and slopes with fixed `gestational_age` at their mean values.
...
used_colors = unique(ggplot_build(gmine)$data[[1]]$colour)
mean_gestational_age = mean(qpcr_data_IFNL$gestational_age)
intercept = coefs["(Intercept)"] + 
  coefs["gestational_age"]*mean_gestational_age
h1n1 = coefs["treatmentH1N1:gestational_age"]*mean_gestational_age
slope = coefs["time_to_delivery:gestational_age"]*mean_gestational_age
gmine = gmine + 
