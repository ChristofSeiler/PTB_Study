...
pkgs_needed = c("oligo","limma","hta20transcriptcluster.db","pd.hta.2.0",
                "affycoretools","genefilter","MatchIt","ggfortify",
...
sample_table %<>% na.omit
sample_table %>% 
  dplyr::select(sample_id,pair,ptb_case,gestage_pbmc_collected,gestage_delivery) %>% 
  arrange(pair,ptb_case) %>% 
  print(n = Inf)
write_csv(sample_table,path = "sample_table_matched.csv")
ggplot(sample_table, aes(as.factor(pair), gestage_pbmc_collected, color = ptb_case)) + 
